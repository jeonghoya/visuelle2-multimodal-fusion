...
def run(args):
    ...
    trainset = Visuelle2(
        ...
        output_len=args.output_len  # [추가] 중요!
    )
    testset = Visuelle2(
        sales_df=test_df,
        img_root=img_folder,
    ...
    if demand:
        model = DemandModel(
            ...
            fab_dict=fab_dict,
            store_num=125, # 매장 수
            num_trends=3,  # 구글 트렌드 수
            use_att=True,  # 속성 사용 여부 (DemandModel은 보통 다 킨다)
            use_date=True, # 날짜 사용 여부
            use_trends=True # 트렌드 사용 여부
        )
    else:
        if args.task_mode == 0:
            print("Initializing Full-Feature 2-1 Model...")
            model = Model21(
                attention_dim=args.attention_dim,
                embedding_dim=args.embedding_dim,
                hidden_dim=args.hidden_dim,
                use_img=args.use_img,
                out_len=args.output_len, # 1이 들어가야 함
                # [추가] 2-10 모델과 동일하게 메타데이터 전달
                cat_dict=cat_dict,
                col_dict=col_dict,
                fab_dict=fab_dict,
                store_num=125,
    ...
    # Print training time
    print(f"[Training Completed] Time: {elapsed/60:.2f} minutes ({elapsed:.2f} seconds)")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--gpu_num", type=int, default=0) # GPU 번호 설정
    ...
    parser.add_argument("--ckpt_dir", type=str, default="ckpt_CrossAttnRNN210/")
