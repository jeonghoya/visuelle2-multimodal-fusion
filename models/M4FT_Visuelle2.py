...
# ==============================================================================
# [Part 3] M4FT Fusion Network (핵심 아키텍처)
# ==============================================================================

class FusionBlock(nn.Module):
    """ Basic MLP Block for Fusion """
...
class M4FTFusionNetwork(nn.Module):
    def __init__(self, hidden_dim, dropout=0.2):
        super().__init__()
        # 3개의 Fusion Network (Weight Sharing 안 함)
        self.fusion_temp_text = FusionBlock(hidden_dim, dropout)
        self.fusion_text_vis = FusionBlock(hidden_dim, dropout)
        self.fusion_final = FusionBlock(hidden_dim, dropout)

    def forward(self, e_temp, e_text, e_vis):
        """
        Input: 3 Original Embeddings (All shape [B, Hidden])
        Logic: Summation based Fusion
        """
        # 1. 1st Layer Fusion (Add & Pass)
        # Temporal + Text
        feat_tt = e_temp + e_text
        out_tt = self.fusion_temp_text(feat_tt)
        
        # Text + Vision
        feat_tv = e_text + e_vis
        out_tv = self.fusion_text_vis(feat_tv)
        
        # 2. Final Fusion (Grand Summation)
        # Output of Sub-fusions + All Original Inputs
        final_input = out_tt + out_tv + e_temp + e_text + e_vis
        
        # Final MLP
        return self.fusion_final(final_input)

# ==============================================================================
# [Part 4] Main Model Class
# ==============================================================================

class M4FT_Visuelle2(pl.LightningModule):
    def __init__(self, embedding_dim, hidden_dim, output_dim, num_heads, num_layers, use_text, use_img, 
        ...
        # Static Features (Linear Projections included to match hidden_dim)
        ...
        # 2. M4FT Fusion Module
        self.fusion_network = M4FTFusionNetwork(hidden_dim)

        # 3. Decoder (Same as GTM)
    ...
    def forward(self, item_sales, category, color, fabric, store, temporal_features, gtrends, images):
        ...
        # 1. Encode Static Features (All projected to hidden_dim)
        ...
        # Repeat for window splits if necessary
        if num_splits > 1:
        ...
        # 2. M4FT Hierarchical Fusion (Summation based)
        static_context = self.fusion_network(e_temp, e_text, e_vis) # [B, Hidden]

        # 3. Encode Dynamic Features
        # Google Trends (Encoder Memory)
        ...
        # 4. Decoder Input (Fusion Output + Sales Context)
