...
class ImageEncoder(nn.Module):
    ...
    def forward(self, x):
        bs = x.size(0) # [수정 1] 배치 사이즈를 캡처
        x = self.cnn(x) # 출력: [Batch, 2048, Height, Width] (여기선 10x10)
        
        # [수정 2] 하드코딩된 view(-1, 64, ...) 대신 배치 사이즈 유지하며 flatten
        # [Batch, 2048, H, W] -> [Batch, 2048, H*W] -> [Batch, H*W, 2048]
        x = x.flatten(2).permute(0, 2, 1) 
        
        x = self.dropout(self.fc(x)) # [Batch, H*W, Embedding_dim]
